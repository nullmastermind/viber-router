...
- **THEN** the system SHALL add the model to the group's allowed list, invalidate the group's Redis cache, and return HTTP 201
...
- **THEN** the system SHALL create the model in the master list (if not exists), add it to the group's allowed list, invalidate the group's Redis cache, and return HTTP 201
...
- **THEN** the system SHALL return HTTP 409 with error message "Model already assigned to this group"
