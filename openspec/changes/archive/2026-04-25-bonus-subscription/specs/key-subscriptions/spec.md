...
- **THEN** the `key_subscriptions` table SHALL have all previously specified columns plus `bonus_base_url`, `bonus_api_key`, `bonus_name`, `bonus_quota_url`, `bonus_quota_headers`, and the `sub_type` CHECK constraint SHALL include 'pay_per_request' and 'bonus'
...
The system SHALL allow assigning a plan to a sub-key via POST `/api/admin/groups/:group_id/keys/:key_id/subscriptions` with body `{ "plan_id": "<uuid>" }`. The system SHALL snapshot all plan values into the subscription record, including `model_request_costs`. Alternatively, the body MAY contain `bonus_name`, `bonus_base_url`, and `bonus_api_key` (without `plan_id`) to create a bonus subscription directly.
