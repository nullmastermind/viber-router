...
- **THEN** the system SHALL query PostgreSQL for the group config (including count-tokens server detail if `count_tokens_server_id` is set), store it in Redis as `group:config:sk-vibervn-xxx`, and use it for routing
