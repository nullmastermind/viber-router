...
- **THEN** the `subscription_plans` table SHALL have all specified columns including `model_request_costs`, `rpm_limit`, and `tpm_limit`, and the `sub_type` CHECK constraint SHALL include 'pay_per_request'

### Requirement: Create subscription plan
The system SHALL allow creating a plan via POST `/api/admin/subscription-plans` with body `{ "name": "<string>", "sub_type": "fixed"|"hourly_reset"|"pay_per_request", "cost_limit_usd": <number>, "model_limits": {"<model_name>": <number>}, "model_request_costs": {"<model_name>": <number>}, "rpm_limit": <number|null>, "tpm_limit": <number|null>, "reset_hours": <number|null>, "duration_days": <number> }`.
...
#### Scenario: Update TPM limit
- **WHEN** an admin patches a plan with `{ "tpm_limit": 120000 }`
- **THEN** the system SHALL update the plan and set `tpm_limit = 120000` on active key subscriptions for that plan

#### Scenario: Clear TPM limit
- **WHEN** an admin patches a plan with `{ "tpm_limit": null }`
- **THEN** the system SHALL update the plan and set `tpm_limit = NULL` on active key subscriptions for that plan

### Requirement: Sync subscription plan TPM
The system SHALL expose POST `/api/admin/subscription-plans/{id}/sync-tpm` to copy a plan's current `tpm_limit` to all active key subscriptions created from that plan and invalidate affected subscription caches.

#### Scenario: Sync TPM to active subscriptions
- **WHEN** an admin calls POST `/api/admin/subscription-plans/{id}/sync-tpm` for a plan with `tpm_limit = 100000`
- **THEN** all active key subscriptions with that `plan_id` SHALL have `tpm_limit = 100000`

#### Scenario: Sync TPM does not update inactive subscriptions
- **WHEN** an admin calls POST `/api/admin/subscription-plans/{id}/sync-tpm`
- **THEN** subscriptions with terminal or inactive statuses SHALL NOT be modified
