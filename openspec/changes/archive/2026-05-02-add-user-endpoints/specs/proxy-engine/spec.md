...
#### Scenario: Priority user endpoint succeeds before bonus
- **WHEN** a request has an enabled, model-compatible priority user endpoint and a bonus subscription is also available
- **THEN** the proxy SHALL attempt the priority user endpoint before bonus servers and return its 2xx response immediately

#### Scenario: Priority user endpoint fails then bonus attempted
- **WHEN** all enabled, model-compatible priority user endpoints return non-2xx or connection errors and bonus servers are available
- **THEN** the proxy SHALL continue to the existing bonus server flow

#### Scenario: Blocked subscription tries fallback endpoints
- **WHEN** subscription enforcement blocks the request because quota is exhausted and an enabled, model-compatible fallback user endpoint exists
- **THEN** the proxy SHALL attempt fallback user endpoints before returning 429

#### Scenario: Group server exhaustion tries fallback endpoints
- **WHEN** subscription enforcement allows the request but all group servers fail
- **THEN** the proxy SHALL attempt fallback user endpoints before returning the final 429

#### Scenario: Fallback endpoint success
- **WHEN** a fallback user endpoint returns a 2xx response
- **THEN** the proxy SHALL return that response immediately instead of returning 429

#### Scenario: All fallback endpoints fail
- **WHEN** subscription enforcement is blocked or group servers are exhausted and all compatible fallback user endpoints fail
- **THEN** the proxy SHALL return the existing path-appropriate 429 response

#### Scenario: User endpoint preserves request path and query
- **WHEN** a request to `/v1/chat/completions?beta=true` is routed to a user endpoint with `base_url = "https://example.test/api"`
- **THEN** the upstream request SHALL target `https://example.test/api/v1/chat/completions?beta=true`

#### Scenario: User endpoint forwards safe headers and credentials
- **WHEN** a request is routed to a user endpoint
- **THEN** the proxy SHALL forward request headers except authentication, host, and content-length headers, and SHALL set both `x-api-key` and `authorization` from the user endpoint API key

#### Scenario: User endpoint streaming usage parser
- **WHEN** a user endpoint returns a streaming 2xx response for `/v1/messages` or `/v1/chat/completions`
- **THEN** the proxy SHALL wrap the stream with the existing Anthropic or OpenAI usage parser based on endpoint type and SHALL forward bytes unchanged to the client

#### Scenario: User endpoint usage logging
- **WHEN** a user endpoint serves a billable 2xx response with extractable usage
- **THEN** the proxy SHALL enqueue token usage with `user_endpoint_id` set, `subscription_id = NULL`, and `server_id = Uuid::nil()` for streaming wrappers that require a server identifier
