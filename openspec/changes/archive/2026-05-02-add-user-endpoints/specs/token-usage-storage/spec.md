...
- **THEN** the system SHALL insert a record into `token_usage_logs` with the group_id, server_id, model, input_tokens, output_tokens, cache token counts, is_dynamic_key flag, key_hash, group_key_id, cost_usd, subscription_id, user_endpoint_id, and content_hash
...
- **THEN** the record SHALL have `subscription_id` set to the charged subscription's UUID and `user_endpoint_id: NULL`
...
#### Scenario: User endpoint usage
- **WHEN** the request was served by a user endpoint
- **THEN** the record SHALL have `user_endpoint_id` set to the serving endpoint UUID and `subscription_id: NULL`
