...
- **THEN** the system SHALL fetch subscriptions from GET `/api/admin/groups/:id/keys/:key_id/subscriptions` and display them in a table with columns: Plan (name from snapshot or plan reference), Type, Budget, Used, Status
