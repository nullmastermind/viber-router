...
The system SHALL return a single group's details including its assigned servers with priorities and model mappings.

#### Scenario: Group exists
- **WHEN** an authenticated admin sends GET `/api/admin/groups/{id}`
- **THEN** the system SHALL return HTTP 200 with the group object including its servers array (each with server details, priority, and model_mappings)
...
The system SHALL allow updating a group's name, failover_status_codes, and is_active status. The Redis cache for this group SHALL be invalidated on update.
