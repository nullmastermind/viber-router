...
The system SHALL cache the full group configuration (group details, server list with priorities and model mappings) in Redis keyed by API key after the first database lookup.
...
- **THEN** the system SHALL query PostgreSQL for the group config, store it in Redis as `group:config:sk-vibervn-xxx`, and use it for routing

#### Scenario: Cache hit
- **WHEN** a proxy request arrives with `x-api-key: sk-vibervn-xxx` and a Redis cache entry exists
- **THEN** the system SHALL use the cached config without querying PostgreSQL

#### Scenario: Redis unavailable — fallback to DB
- **WHEN** Redis is unavailable (connection error)
- **THEN** the system SHALL fall back to querying PostgreSQL directly and continue serving the request
...
- **WHEN** a group's name, failover_status_codes, or is_active is updated
...
- **THEN** the Redis cache entries for ALL groups that reference this server SHALL be deleted
