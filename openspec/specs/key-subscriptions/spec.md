...
- **THEN** the `key_subscriptions` table SHALL have all previously specified columns plus `weekly_cost_limit_usd`, `rpm_limit`, `tpm_limit`, `bonus_base_url`, `bonus_api_key`, `bonus_name`, `bonus_quota_url`, `bonus_quota_headers`, and the `sub_type` CHECK constraint SHALL include 'pay_per_request' and 'bonus'
...
The system SHALL allow assigning a plan to a sub-key via POST `/api/admin/groups/:group_id/keys/:key_id/subscriptions` with body `{ "plan_id": "<uuid>" }`. The system SHALL snapshot all plan values into the subscription record, including `weekly_cost_limit_usd`, `model_request_costs`, `rpm_limit`, and `tpm_limit`. Alternatively, the body MAY contain `bonus_name`, `bonus_base_url`, and `bonus_api_key` (without `plan_id`) to create a bonus subscription directly.

#### Scenario: Assign plan to sub-key
- **WHEN** an admin sends POST with `{ "plan_id": "<uuid>" }` referencing an active plan
- **THEN** the system SHALL create a `key_subscriptions` record with `sub_type`, `cost_limit_usd`, `weekly_cost_limit_usd`, `model_limits`, `model_request_costs`, `rpm_limit`, `tpm_limit`, `reset_hours`, `duration_days` copied from the plan, `status: "active"`, `activated_at: NULL`, and return it with status 201
...
#### Scenario: Assign plan snapshots weekly limit
- **WHEN** an admin assigns a plan with `weekly_cost_limit_usd: 40.0`
- **THEN** the created `key_subscriptions` record SHALL have `weekly_cost_limit_usd: 40.0` copied from the plan
